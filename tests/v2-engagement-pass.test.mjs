...
test('engagement bank is a non-authoritative immutable successor',()=>{
 ...
 assert.equal(bank.status,'PROPOSED_FROZEN_FOR_OWNER_EXPERIENCE_REVIEW');
...
test('engagement edits preserve all admitted mappings and measurement metadata',()=>{
 ...
 for(const after of bank.questions){
  ...
  for(const key of ['format','color','domain','family','construct']) assert.deepEqual(after[key],before[key],`${after.id}:${key}`);
  assert.deepEqual(after.options.map(o=>o.color),before.options.map(o=>o.color),after.id);
  if(after.engagement_revision){
   assert.equal(after.engagement_revision.measurement_change,false);
   assert.equal(after.engagement_revision.mapping_change,false);
   assert.equal(after.engagement_revision.scoring_change,false);
   assert.equal(after.engagement_revision.semantic_family_change,false);
  }
 }
...
test('option distinctness audit records resolved and unresolved cases explicitly',()=>{
 ...
 assert.equal(audit.mechanical_option_parallelism.unresolved.length,2);
 for(const id of audit.mechanical_option_parallelism.unresolved) assert.ok(audit.mechanical_option_parallelism.unresolved_rationale[id]);
});
