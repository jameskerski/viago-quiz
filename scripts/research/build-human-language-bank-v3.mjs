...
const prompts={
 ...
 'C03-L-Y-01@validation-1.0.0':'When a new opportunity means leaving a familiar group, I give real weight to the relationships I have built.',
 ...
 'EXP1-L-G-01@validation-1.0.0':'I trust an explanation more when I can see its evidence and assumptions.',
 ...
 'EXP2-L-G-01@validation-1.0.0':'A success feels more useful after I understand what made it repeatable.',
 'EXP2-L-R-02@validation-1.0.0':'When closure and exploration both make sense, I lean toward visible progress sooner.',
 'EXP2-L-G-02@validation-1.0.0':'Recognition matters more for rigor and reliability than for the size of my result.',
...
const humanLoadOverrides=new Set([
 ...
 'EXP1-L-G-02@validation-1.0.0','EXP2-L-R-01@validation-1.0.0','EXP3-L-R-011@validation-2.0.0','EXP3-L-R-012@validation-2.0.0','EXP3-L-B-002@validation-2.0.0'
]);
const revisionId=question=>`${question.id}@human-1.0.0`;
const questions=source.questions.map(current=>{
 ...
 const optionLabels=calibrated?.options;
 ...
 return {...current,question_revision_id:revisionId(current),prompt,options,revision:{source_question_revision_id:current.question_revision_id,language_standard:'VIAGO_HUMAN_LANGUAGE_STANDARD_V1',changed:prompt!==current.prompt||options.some((option,index)=>option.label!==current.options[index].label),measurement_change:false,mapping_change:false,scoring_change:false,semantic_family_change:false,current_load:load(current),proposed_load:proposedLoad,load_review:proposedLoad===computedLoad?'RULE_BASED':'HUMAN_REVIEWED_OVERRIDE',current_prompt_words:wordCount(current.prompt),proposed_prompt_words:wordCount(prompt),current_option_spread:optionSpread(current),proposed_option_spread:optionSpread({...current,options})}};
});
...
const successor={schema_version:'3.0.0',bank_version:'viago-validation-bank-293-human-v3.0.0',status:'PROPOSED_FROZEN_FOR_OWNER_READABILITY_REVIEW',semantic_authority:'VIAGO_BEHAVIORAL_COLOR_MODEL_V1_0',language_authority:'VIAGO_HUMAN_LANGUAGE_STANDARD_V1',parent_bank_version:source.bank_version,parent_bank_hash:source.bank_hash,question_count:questions.length,formats:source.formats,assembler_compatibility:'viago-validation-assembler-v2.0.0',scoring_compatibility:'viago-validation-scoring-equal-opportunity-v1.0.0',questions};
...
fs.writeFileSync('data/v2-research/validation-bank-293-human-v3.0.0.json',JSON.stringify(successor,null,2)+'\n');
fs.writeFileSync('data/v2-research/human-language-full-bank-review-v1.0.0.json',JSON.stringify(review,null,2)+'\n');
...
fs.writeFileSync('docs/v2-human-language-full-bank-review-v1.0.0.md',reviewMarkdown);
